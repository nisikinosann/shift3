...
const helpContent = {
    ...
    group: { title: "グループ設定について", content: `<p>関連業務をまとめる機能.</p><p><strong>グループ名:</strong> 例：キッチン、ホール.</p><p><strong>グループの間隔:</strong> このグループの業務を一度行うと、次にこのグループの<strong>いずれかの</strong>業務ができるまでの最短日数. 例えば、「キッチン」グループの間隔を「3」と設定した場合、1月1日にキッチンの業務を行った従業員は、1月2、3、4日はキッチンの業務ができず、次にできるのは1月5日となります.</p>` },
...
function collectJobData(isForSave = false) {
    ...
    if (mode === 'b' && !isForSave) {
        ...
        return jobEntries.map(entry => {
            ...
            let name, group;
            if (match) { name = match[1].trim(); group = match[2].trim(); } else { name = entry.trim(); group = ''; }
            return { name, group, interval: 0, people: 1 };
        });
    }
    const jobs = [];
...
function checkAvailability(employee, job, date, employeeLastJob, groups) {
    ...
    const lastDateForThisJob = lastJobRecords.jobs[job.name];
    if (lastDateForThisJob && job.interval > 0) {
        if ((currentDate - new Date(lastDateForThisJob)) / 86400000 < job.interval) return false;
    }
    if (job.group) {
        const groupInfo = groups.find(g => g.name === job.group);
        if (groupInfo && groupInfo.interval > 0) {
            const lastDateForThisGroup = lastJobRecords.groups[job.group];
            if (lastDateForThisGroup) {
                if ((currentDate - new Date(lastDateForThisGroup)) / 86400000 < groupInfo.interval) return false;
            }
        }
    }
    return true;
...
function handleCellEdit(event) {
    ...
    if (newValue === '' || newValue === '休') {
        ...
        cell.textContent = '休';
        cell.className = cell.className.replace('vacation', '').trim();
    } else if (newValue === '休暇') {
...
function exportToExcel() {
    ...
    for (const date in shiftData) { for (const name in shiftData[date]) { employeeSet.add(name); } }
    const employees = Array.from(employeeSet).filter(name => !name.includes('休業日'));
    const data = [];
